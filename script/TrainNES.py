...
def train():
    def Error(z):
        ...
        xt = x_data * w
        z = z * w
        e = xp.sqrt(xp.sum((xt-z)**2,axis=1))
        return e
    ...
    class SimNES(FunctionNode):
        def forward_cpu(self, inputs):
            v, w = inputs
            self.R = 1
            self.sigma = .01
            self.p = perturbation(self.R, self.sigma)
            vp = v + self.p[:,0]
            vn = v - self.p[:,0]
            wp = w + self.p[:,1]
            wn = w - self.p[:,1]
            Command(v_wait, w_wait, draw_path=False)
            e_up = Error(Command(vp, wp))
            Command(v_wait, w_wait, draw_path=False)
            e_down = Error(Command(vn, wn))
            self.D = e_up - e_down
            ret = self.D
            return ret,
        ...
        def backward_cpu(self, inputs, grad_outputs):
            v, w = inputs
            gw, = grad_outputs
            gv = self.D * self.p[:,0] * 0.5 / (self.sigma**2 * self.R)
            gw = self.D * self.p[:,1] * 0.5 / (self.sigma**2 * self.R)
            return gv, gw
        ...
        def forward_gpu(self, inputs):
            ...
            self.R = 1
            self.sigma = .01
            self.p = cp.asarray(perturbation(self.R, self.sigma))
            vp = v + self.p[:,0]
            vn = v - self.p[:,0]
            wp = w + self.p[:,1]
            wn = w - self.p[:,1]
    ...
    dirname = 'SimNES_' + os.path.dirname(argv[1])
    os.mkdir(dirname)
    Command(v_wait, w_wait, draw_path=False)
    try:
        while not rospy.is_shutdown():
            ...
            e = simnes(v,w)
            D_loss = D_loss + xp.sum(e.data)
            model.cleargrads()
            e.backward()
            opt.update()
            counter = counter + 1
            print('itr:', counter, ' D:',xp.sum(e.data))
